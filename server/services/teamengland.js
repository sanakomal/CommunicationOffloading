  ...
  return TeamEngland.find((err, teamengland) => {
    if (!err) {
      return map(results.Player, (player) => {
        let teameng = find(teamengland, { TeamId: player.personid });
        if (isUndefined(teameng)) { 
          const latestTeam = new TeamEngland({
          ...
          if (teameng) {
          TeamEngland.findOneAndUpdate({ 'TeamId': teameng.personid }, {
            $set: {
             TeamId: player.personid,
              FirstName: player.FirstName,
              LastName: player.LastName,
